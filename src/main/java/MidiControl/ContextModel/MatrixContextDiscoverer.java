...
public class MatrixContextDiscoverer implements ContextDiscoverer {
    private static final Logger log = Logger.getLogger(MatrixContextDiscoverer.class.getName());
    
    private static final Pattern MATRIX_PATTERN =
        Pattern.compile("^kMatrix(?<matrix>\\d+)(?<param>[A-Za-z]+)$");
    
    @Override
    public void discover(List<Context> out, CanonicalRegistry registry) {
        log.info("--- Discovering Matrices ---");

        Map<String, List<MixMapping>> matrixMap = new HashMap<>();
        Map<String, String> matrixLabels = new HashMap<>();

        for (ControlGroup group : registry.getGroups().values()) {

            String groupName = group.getName();

            // Only groups that can legitimately contain matrix sends
            if (!(groupName.contains("ToMatrix") || groupName.startsWith("kMatrix")))
                continue;

            for (SubControl sub : group.getSubcontrols().values()) {

                Matcher m = MATRIX_PATTERN.matcher(sub.getName());
                if (!m.matches())
                    continue;

                String matrixDigits = m.group("matrix");
                String param        = m.group("param");

                BusInfo bus = parseBusDigits(matrixDigits, "Matrix");
                if (bus == null) {
                    log.fine("Ignoring unparseable matrix index '" + matrixDigits +
                             "' from subcontrol " + sub.getName());
                    continue;
                }

                String contextSuffix = bus.contextSuffix();   // "3" or "3-4"
                String label         = bus.displayLabel();    // "Matrix 3" or "Matrix 3/4"

                String prefix = sub.getName().substring(
                    0, sub.getName().length() - param.length()
                );

                matrixMap.computeIfAbsent(contextSuffix, k -> new ArrayList<>())
                         .add(new MixMapping(groupName, prefix));

                matrixLabels.putIfAbsent(contextSuffix, label);

                log.fine("Matrix send detected: group=" + groupName
                    + " sub=" + sub.getName()
                    + " → " + label + " (suffix=" + contextSuffix + ")");
            }
        }

        for (var entry : matrixMap.entrySet()) {
            String suffix = entry.getKey();
            List<MixMapping> mappings = entry.getValue();
            String label = matrixLabels.getOrDefault(suffix, "Matrix " + suffix);
            ...
            String contextId = "matrix." + suffix;
