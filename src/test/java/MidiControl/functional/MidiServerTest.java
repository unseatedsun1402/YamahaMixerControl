...
public class MidiServerTest {
  ...
  void testAddToSendQueue() {
    ...
    org.junit.jupiter.api.Assertions.assertEquals(
        before + 1, after, "Buffer should have one more element after adding");
  }
  ...
  void testInputReceiverReceivesMessage() {
    ...
    try (MidiInputReceiver receiver = new MidiInputReceiver(inputBuffer)) {
      ...
      org.junit.jupiter.api.Assertions.assertFalse(
          inputBuffer.isEmpty(), "Input buffer should not be empty after receiving a message");
      org.junit.jupiter.api.Assertions.assertEquals(
          1, inputBuffer.size(), "Input buffer should contain exactly one message");
      org.junit.jupiter.api.Assertions.assertEquals(
          msg, inputBuffer.poll(), "Received message should match sent message");
    }
  }
  ...
  public void testNrpnJsonStructureWithHelpers() {
    NrpnMapping mapping =
        new NrpnMapping(1, 0, ControlType.FADER, ChannelType.MIX, 3, "Mix 9 send 3 control");
  ...
  void testSetOutputDeviceInitializesReceiver() throws Exception {
    ...
    assertDoesNotThrow(
        () -> MidiServer.midiOut.sendMessage(testMessage),
        "Should send MIDI message without error");
  }
  ...
  void testSetInputDeviceWithOutputIndexFailsGracefully() {
    ...
    assertTrue(
        ...
        "Device should not support transmitters (not suitable for input)");
    assertDoesNotThrow(
        () -> {
          try {
