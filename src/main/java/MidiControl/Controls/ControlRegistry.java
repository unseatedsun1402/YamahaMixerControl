...
public class ControlRegistry {
  ...
  public ControlRegistry(List<SysexMapping> mappings) {
    ...
    Map<Integer, List<SysexMapping>> grouped =
        mappings.stream().collect(Collectors.groupingBy(SysexMapping::getControl_id));
  ...
  /**
   * Resolve a SysEx message to its Control group. Uses SysexRegistry to find the mapping, then
   * returns the Control for that mapping’s controlId.
   */
