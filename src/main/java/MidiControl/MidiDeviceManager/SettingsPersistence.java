...
public class SettingsPersistence {
    ...
    private static File resolveSettingsFile() {
        ...
        if (!settingsDir.canWrite()) {
            Logger.getLogger(SettingsPersistence.class.getName()).warning("Settings dir not writable: " + settingsDir);
            settingsDir = new File(System.getProperty("java.io.tmpdir"), SUBDIR);
            settingsDir.mkdirs();
        }

        File settingsFile = new File(settingsDir, SETTINGS_FILENAME);
        Logger.getLogger(SettingsPersistence.class.getName()).info("Resolved SETTINGS_PATH: " + settingsFile.getAbsolutePath());
        return settingsFile;
    }
    ...
    public static DeviceSettings load() {
        ...
        try (FileReader reader = new FileReader(file)) {
            ...
            Logger.getLogger(SettingsPersistence.class.getName()).log(Level.WARNING, "Failed to load settings", e);
    ...
    public static void save(DeviceSettings settings) {
        ...
        try (Writer writer = new FileWriter(file)) {
            ...
            Logger.getLogger(SettingsPersistence.class.getName()).info("Saving settings: " + new Gson().toJson(settings));
        } catch (IOException e) {
            Logger.getLogger(SettingsPersistence.class.getName()).log(Level.WARNING, "Failed to save settings", e);
        }
    }
}